...
class CO2room{
    static name = "CO2 room module";
	static icon= "/icon.svg"; 		// relative to: /drivers/<driver_id>/assets/
	static capabilities = ["measure_flow", "measure_temperature", "measure_flow", "target_flow", "operational_status", "operational_status.type", "measure_co2", "target_co2"];
	static capabilitiesOptions= { };
	


	static lib_url = "../../lib/co2room.js";
...
*/
		...
		this.client 	= new CO2room.ModbusRTU();	
		console.log("initiated duco device: " + CO2room.name + "@" +this.duco_IPaddress+":"+this.duco_IPport+"->"+this.duco_modbusID+"["+this.duco_node +"]");
    ...
    handler(self){  //self.log("####### DEVICE_LIB CODE ######");					
						...
						this.client.connectTCP(this.duco_IPaddress, { port: this.duco_IPport }).then(() =>{					
							...
							Promise.all([
								this.client.readInputRegisters(10 * this.duco_node, 2),		//#0:type, #1:status, 
								this.client.readInputRegisters(10 * this.duco_node + 3, 2), //#3:temperature, #4:co2_level
								this.client.readInputRegisters(10 * this.duco_node + 9, 1), //#9
								
								this.client.readHoldingRegisters(10 * this.duco_node, 2),		// 						
								this.client.readHoldingRegisters(10 * this.duco_node + 4, 4),	//
								...
								this.client.readInputRegisters(19, 1) //Node Group
									...
									this.flow_setp		= data[3].data[0];	//input 13									
									this.co2_setp		= data[3].data[1];	//input 13									
									
									this.man_low_level	= data[4].data[0];
									this.man_mid_level 	= data[4].data[1];	//input 15
									this.man_high_level = data[4].data[2];	//input 15
									this.man_time 		= data[4].data[3];	//input 15
...
exports = module.exports = CO2room;
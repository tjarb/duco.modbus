...
class Reg_room{
    static name = "Sensorless room module";
	static icon= "/icon.svg"; 		// relative to: /drivers/<driver_id>/assets/
	static capabilities = ["measure_temperature", "operational_status", "operational_status.type", "target_flow"];
	static capabilitiesOptions= { };
	
	static lib_url = "../../lib/reg_room.js";
...
*/
		...
		this.client 	= new Reg_room.ModbusRTU();	
		console.log("initiated duco device: " + Reg_room.name + "@" +this.duco_IPaddress+":"+this.duco_IPport+"->"+this.duco_modbusID+"["+this.duco_node +"]");
						...
						this.client.connectTCP(this.duco_IPaddress, { port: this.duco_IPport }).then(() =>{					
							...
							Promise.all([
								...
								this.client.readInputRegisters(19, 1) //Node Group
									...
									this.man_mid_level 	= data[4].data[1];	//input x4
									this.man_high_level = data[4].data[2];	//input x5
									this.man_time 		= data[4].data[3];	//input x6
									
									this.node_action	= data[5].data[0];	//input x9									
...
exports = module.exports = Reg_room;
...
class RHValve{
	...
	static capabilities = ["flow_level", "target_flow", "operational_status", "operational_status.type"];
	static capabilitiesOptions= { };
	
 	static lib_url = "../../lib/contact.js";
...
//        this._changedCurrent_L1 = self.homey.flow.getDeviceTriggerCard('changedCurrentL1');
    ...
    handler(self){  //self.log("####### DEVICE_LIB CODE ######");					
						...
						this.client.connectTCP(this.duco_IPaddress, { port: this.duco_IPport }).then(() =>{					
							...
							Promise.all([
								this.client.readInputRegisters(10 * this.duco_node, 3),		//#0:type, #1:status, #2ventlevel								
								this.client.readInputRegisters(10 * this.duco_node + 9, 1), //#9:node-group
								
								this.client.readHoldingRegisters(10 * this.duco_node, 1),		// 						
								this.client.readHoldingRegisters(10 * this.duco_node + 2, 2),		//
								...
								this.client.readInputRegisters(19, 1) //Node Group
									...
									this.flow_setp		= data[2].data[0];	//input 13									
									
									this.mode			= switch_mode( data[3].data[0] );	//input 13		
									this.value			= data[3].data[0];	//input 13		
									
									this.node_action	= data[4].data[0];	//input 19
...
*/
	...
	operational_string(operational_code){
		...
		switch_mode(operational_code){
		
		switch(operational_code){						
				case 0: return "OVERRULE";
				case 1: return "HEATPUMP";
